...
dataframe = df_fill[(df_fill['Y house price of unit area'] <= 80) & (df_fill['X3 distance to the nearest MRT station'] <= 2800) & (df_fill['X6 longitude'] >= 121.50)] # YOUR CODE HERE
...
dataframe['Y house price of unit area'] = dataframe['Y house price of unit area'] * 91# YOUR CODE HERE

# Perform Normalization on the data (hint: check MinMaxScaler()) -- report the mean of the Y column after this

from sklearn.preprocessing import MinMaxScaler
scaler = MinMaxScaler()
normalized = scaler.fit_transform(dataframe) # YOUR CODE HERE
...
# Subset the reindex_df querying for House Age less than or equal to 9 years and Price of Unit Area greater than 27
...
mean_val_conv_7 = round(grouped_age_price_df.loc[7], 2) # Put the value here -- 1.17 of Gradescope tests
...
# YOUR CODE HERE
grouped_age_price_df.plot(kind='bar', title='Mean House Price based on Convenience store proximity', ylabel='Price of unit area', xlabel='Number of convenience stores', figsize=(6, 5))
plt.show()
